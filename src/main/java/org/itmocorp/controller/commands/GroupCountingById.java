...
public class GroupCountingById extends AbstractCommand {
    ...
    public void execute() {
        if (args.length == 0) {
            CommandManager.printToClient("Была вызвана команда GroupCountingById");
            ...
            for (Integer key : idMap.keySet()){
                CommandManager.printToClient("Кол-во объектов с id=" + key + ": " + idMap.get(key));
            }
            CommandManager.printToClient("Команда GroupCountingById завершила свое выполнение");
        }else{
